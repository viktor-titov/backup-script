...
sources=$(jq -r '.sources[]' "$setting")
if [[ -z $sources ]]; then 
    msg "$(tf "В файле $setting нет поля 'source', указывающее источники для архивации." "red")"
    exit 1
fi

for source in $sources; do
    if [ ! -r "$source" ]; then
        msg "$(tf "Файл или папка $source не существует. Проверьте настройки в файле .settingScript.json" "red")"
        exit 1
    fi
done
...
for source in $sources; do
    archive $source "$(basename $source)"
done
